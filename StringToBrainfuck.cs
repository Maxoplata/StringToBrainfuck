/**
 * StringToBrainfuck.cs
 *
 * Converts a string to a Brainfuck script that will output said string.
 * usage: csc StringToBrainfuck.cs && ./StringToBrainfuck your string here
 *
	...
	static void Main(string[] args) {
		...
		if (args.Length > 0) {
			...
			for (int i = 0; i < inputString.Length; i++) {
				...
				int charVal = (int) inputString[i];

				if (charVal > currentLocation) {
					// move ahead on the "tape" to build the character
					bfCode = bfCode + new string('+', (int) Math.Floor((decimal) ((charVal - currentLocation) / 10)));
					bfCode = bfCode + "[>++++++++++<-]>";
					bfCode = bfCode + new string('+', (charVal - currentLocation) % 10);
				} else if (charVal < currentLocation) {
					// move backwards on the "tape" to build the character
					bfCode = bfCode + new string('+', (int) Math.Floor((decimal) ((currentLocation - charVal) / 10)));
					bfCode = bfCode + "[>----------<-]>";
					bfCode = bfCode + new string('-', (currentLocation - charVal) % 10);
				} else {
